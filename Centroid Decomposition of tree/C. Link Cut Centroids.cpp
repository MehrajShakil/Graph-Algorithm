...
int main()
{
   ...
   if ( centroid.size() == 1 ){      /// If there is only one centroid then of course there is an unique centroid.
      cout << e[1][0] << " " << 1 << "\n";
      cout << e[1][0] << " " << 1 << "\n";
   }
   else{    /// centroids must be connect with an edge.
            dfs ( centroid[0] , centroid[1] ); /// looking an edge.
            cout << centroid[1] << " " << x << "\n";
   }
